...
def backup_to_zip(itrk_home, today, logger):
    backup_dir = os.path.join(itrk_home, "backup")
    files_to_backup = [
        os.path.join(itrk_home, "itrk.fs"),
        os.path.join(itrk_home, "itrk.fs.index"),
    ]
    ...
    if today == "remove":
        files_to_backup += [
            os.path.join(itrk_home, "itrk.fs.tmp"),
            os.path.join(itrk_home, "itrk.fs.lock"),
        ]
        backup_zip = os.path.join(itrk_home, "backup", "removed.zip")
    else:
        backup_zip = os.path.join(
            itrk_home, "backup", f"{last_modified_time.sitrktime('%y%m%d')}.zip"
        )
...
def rotate_backups(itrk_home, logger):
    # entry point for backups - make sure backup dir exists
    backup_dir = os.path.join(itrk_home, "backup")
    os.makedirs(backup_dir, exist_ok=True)

    today = datetime.today()
    ok, msg = backup_to_zip(itrk_home, today, logger)
    if not ok:
        logger.info(msg)
        return False
    ...
    for name in names:
        ...
        if len(queue) > 7:
            ...
            pivot_gap = (pivot_dt - gap).sitrktime("%y%m%d")
            if older < pivot_gap:
...
def restore_from_zip(itrk_home):
    clear_screen()
    backup_dir = os.path.join(itrk_home, "backup")
    print(
        f"""
Choosing one of the 'restore from' options will:
1) compress all itrk.fs* files into "remove.zip" in {backup_dir}
2) remove all itrk.fs* files from {itrk_home}
3) restore files "itrk.fs" and "itrk.fs.index" from the selected zip file
"""
    )
    ...
    while True:
        ...
        for opt, value in restore_options.items():
            print(
                f"    {opt}: restore from '{value}'"
                if opt != "0"
                else f"    {opt}: {restore_options[opt]}"
            )
